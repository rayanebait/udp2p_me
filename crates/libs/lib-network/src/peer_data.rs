...
impl PeerData {
    ...
    pub async fn search_good_addr(
        &self,
        sock: UdpSocket,
        pending: &PendingResponseIds,
    ) -> Result<(), PeerError> {
        let addresses = self.socketaddrs.as_ref().unwrap();

        let mut wait_response = FuturesUnordered::new();
        for addr in addresses {
            wait_response.push(handshake_addr(&sock, addr, &pending));
        }
    ...
    pub fn is_peer_addr(&self, addr: SocketAddr) -> bool {
        let peer_has_addr = self.socketaddrs.as_ref().unwrap().contains(&addr);
